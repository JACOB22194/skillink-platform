...
const ForgotPasswordPage: React.FC = () => {
  ...
  const handleVerifyOTP = async () => {
    ...
    setLoading(true); setError(null);
    try {
      // Verify the OTP is valid before asking for new password
  ...
  const handleResetWithToken = async () => {
    ...
    try {
      ...
      const e = err as AxiosError<ApiError>;
      setError(e.response?.data?.detail ?? "Failed to reset password. Link may have expired.");
    } finally { setLoading(false); }
  };
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: c.bg, fontFamily: "sans-serif", padding: "2rem" }}>
      <div style={{ background: c.surface, border: `0.5px solid ${c.border}`, borderRadius: 16, padding: "2.5rem 2rem", width: "100%", maxWidth: 420 }}>
        ...
        {screen === "choose" && (
          <>
            <div style={{ textAlign: "center", marginBottom: "1.75rem" }}>
              ...
              <h1 style={{ fontSize: 20, fontWeight: 500, color: c.text, margin: "0 0 6px" }}>Reset your password</h1>
              <p style={{ fontSize: 13, color: c.subtext, margin: 0 }}>Enter your email, then choose how you'd like to reset your password.</p>
            </div>

            <div style={{ marginBottom: "1.25rem" }}>
              <label style={{ display: "block", fontSize: 13, fontWeight: 500, color: c.text, marginBottom: 6 }}>Email address</label>
              <input
            ...
            {/* Method choice cards */}
            <div style={{ marginBottom: "1.25rem" }}>
              <div style={{ fontSize: 13, fontWeight: 500, color: c.text, marginBottom: 10 }}>How would you like to reset it?</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                >
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 500, color: c.text, marginBottom: 3 }}>Send me a reset link</div>
                    <div style={{ fontSize: 12, color: c.subtext }}>We'll email you a secure link. Click it to set a new password.</div>
                  </div>
                </div>
                ...
                >
                  <div style={{ fontSize: 22, flexShrink: 0, marginTop: 2 }}>🔢</div>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 500, color: c.text, marginBottom: 3 }}>Send me a 6-digit code</div>
                    <div style={{ fontSize: 12, color: c.subtext }}>We'll email you a one-time code. Enter it here to reset your password.</div>
                  </div>
                </div>
            ...
            {loading && (
              <div style={{ textAlign: "center", fontSize: 13, color: c.subtext, padding: "8px 0" }}>Sending…</div>
            )}
        ...
        {screen === "email-sent" && (
          <>
            <div style={{ textAlign: "center", marginBottom: "1.75rem" }}>
              ...
              <h1 style={{ fontSize: 20, fontWeight: 500, color: c.text, margin: "0 0 8px" }}>Check your inbox</h1>
            ...
            <div style={{ background: c.bg, border: `0.5px solid ${c.border}`, borderRadius: 10, padding: "12px 14px", marginBottom: "1.25rem" }}>
              <div style={{ fontSize: 12, color: c.subtext, lineHeight: 1.6 }}>
                ⏱ The link expires in <strong style={{ color: c.text }}>15 minutes</strong>. 
        ...
        {screen === "otp-sent" && (
          <>
            <div style={{ textAlign: "center", marginBottom: "1.75rem" }}>
              <div style={{ width: 48, height: 48, borderRadius: "50%", background: c.primarySoft, color: c.primary, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 12px", fontSize: 22 }}>🔢</div>
              <h1 style={{ fontSize: 20, fontWeight: 500, color: c.text, margin: "0 0 6px" }}>Enter the code</h1>
              <p style={{ fontSize: 13, color: c.subtext, margin: 0, lineHeight: 1.6 }}>
        ...
        {screen === "new-password" && (
          <>
            <div style={{ textAlign: "center", marginBottom: "1.75rem" }}>
              <div style={{ width: 48, height: 48, borderRadius: "50%", background: c.primarySoft, color: c.primary, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 12px", fontSize: 22 }}>🔒</div>
              <h1 style={{ fontSize: 20, fontWeight: 500, color: c.text, margin: "0 0 6px" }}>Set new password</h1>
              <p style={{ fontSize: 13, color: c.subtext, margin: 0 }}>Choose a strong password for your account.</p>
            </div>

            <div style={{ marginBottom: "1rem" }}>
              <label style={{ display: "block", fontSize: 13, fontWeight: 500, color: c.text, marginBottom: 6 }}>New password</label>
              <PasswordInput value={newPassword} onChange={setNewPassword} placeholder="Min. 8 chars, 1 uppercase, 1 number" style={inputStyle} />
            </div>

            <div style={{ marginBottom: "1.25rem" }}>
              <label style={{ display: "block", fontSize: 13, fontWeight: 500, color: c.text, marginBottom: 6 }}>Confirm new password</label>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: "1.25rem" }}>
              {[
                ...
                { label: "Matches",   ok: newPassword.length > 0 && newPassword === confirmPassword },
              ].map(({ label, ok }) => (
        ...
        {screen === "done" && (
          <div style={{ textAlign: "center" }}>
            ...
            <h1 style={{ fontSize: 20, fontWeight: 500, color: c.text, margin: "0 0 8px" }}>Password updated!</h1>
