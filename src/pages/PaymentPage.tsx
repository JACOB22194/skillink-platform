  ...
  return (
    <div style={{ minHeight: "100vh", background: c.bg, fontFamily: "sans-serif", color: c.text }}>
      ...
      <div style={{ maxWidth: 880, margin: "0 auto", padding: "3rem 2rem", display: "grid", gridTemplateColumns: "1fr 360px", gap: 32, alignItems: "start" }}>
        ...
        {/* ── Left: Payment form ── */}
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 600, letterSpacing: "-0.5px", marginBottom: 4 }}>Complete your order</h1>
          <p style={{ fontSize: 13, color: c.subtext, marginBottom: 28 }}>Secure payment powered by Stripe. Cancel anytime.</p>

          {/* Card details */}
          <div style={{ background: c.surface, border: `0.5px solid ${c.border}`, borderRadius: 16, padding: 24, marginBottom: 16 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: c.text, marginBottom: 20 }}>Payment details</div>
          ...
          <div style={{ display: "flex", gap: 16, marginBottom: 20 }}>
            {["🔒 SSL encrypted", "✓ Stripe secured", "↩ Cancel anytime"].map(t => (
              <span key={t} style={{ fontSize: 11, color: c.subtext }}>{t}</span>
            ))}
          </div>
          ...
          <button
            ...
            style={{
              ...
              background: loading ? "#555" : c.primary, color: "#fff",
              fontSize: 15, fontWeight: 600, cursor: loading ? "not-allowed" : "pointer",
              fontFamily: "inherit", transition: "opacity 0.15s",
            }}
          >
            {loading ? "Processing…" : `Pay $${billing === "yearly" ? price * 12 : price}${billing === "yearly" ? " / year" : " / month"}`}
          </button>
        </div>

        {/* ── Right: Order summary ── */}
        <div style={{ background: c.surface, border: `0.5px solid ${c.border}`, borderRadius: 16, padding: 24, position: "sticky", top: 24 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: c.text, marginBottom: 20 }}>Order summary</div>
          ...
          <div style={{ background: c.primarySoft, borderRadius: 12, padding: 16, marginBottom: 20 }}>
            <div style={{ fontSize: 11, color: c.primary, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4 }}>
              {roleType === "freelancer" ? "Freelancer" : "Client"} · {plan.name}
            </div>
            <div style={{ fontSize: 26, fontWeight: 600, color: c.text, letterSpacing: "-0.5px" }}>
              ${price}<span style={{ fontSize: 13, color: c.subtext, fontWeight: 400 }}>/mo</span>
            </div>
            {annualTotal && (
          ...
          {/* Features included */}
          <div style={{ fontSize: 12, fontWeight: 500, color: c.subtext, marginBottom: 12, textTransform: "uppercase", letterSpacing: "0.08em" }}>Includes</div>
          ...
          <div style={{ borderTop: `0.5px solid ${c.border}`, paddingTop: 16 }}>
            ...
            {billing === "yearly" && (
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 8 }}>
                <span style={{ color: "#22c55e" }}>Yearly discount</span>
...
// Helper to reference monthly price for discount display
