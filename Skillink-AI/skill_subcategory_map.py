...
def skills_to_subcategories(skills: list[str], top_languages: list[str] = None) -> list[str]:
    ...
    Args:
        skills:         from parsed["skills"]  (Gemini-extracted)
        top_languages:  from github_stats["top_languages"] (GitHub API)

    Returns:
        Sorted list of unique sub-category names
    """
...
def build_profile_text(parsed: dict) -> str:
    ...
    This is the 'document' that gets compared against job descriptions
    at match time using cosine similarity.
    """
    parts = []

    # Title (3× weight via repetition)
    ...
    # Skills (2× weight via repetition)
    skills = parsed.get("skills", [])
    if skills:
        skill_text = " ".join(skills)
        parts.extend([skill_text] * 2)

    # Experience: project titles, descriptions, tech stacks
...
if __name__ == "__main__":
    # Quick test
    test_skills = ["Python", "React", "FastAPI", "PostgreSQL", "Docker", "Machine Learning"]
    test_langs  = ["Python", "JavaScript", "TypeScript"]
    cats = skills_to_subcategories(test_skills, test_langs)
    print("Sub-categories:", cats)

    test_profile = {
        "title": "Full Stack ML Engineer",
        "summary": "Building data-driven web applications.",
        "skills": test_skills,
        "experience": [{"title": "ML API", "description": "REST API for ML models", "tech_stack": ["Python", "FastAPI"]}],
        "github_stats": {"top_languages": test_langs}
    }
    text = build_profile_text(test_profile)
    print("Profile text:", text[:200])
