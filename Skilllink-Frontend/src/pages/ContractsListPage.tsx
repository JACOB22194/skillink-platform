...
export const ContractsListPage: React.FC = () => {
  ...
  const statusCfg = {
    active:    { color: T.green,  bg: T.greenSoft,  label: t("contract.st.active"),    icon: "⚡" },
    completed: { color: T.accent, bg: T.accentSoft, label: t("contract.st.completed"), icon: "✓" },
    disputed:  { color: T.red,    bg: T.redSoft,    label: t("contract.st.disputed"),  icon: "⚠" },
  };
  ...
  const filterLabels: Record<string, string> = {
    all: t("common.loading").replace("...", "").trim() === "" ? "All" : "All",
  ...
  return (
    <div dir={isRTL ? "rtl" : "ltr"} style={{ minHeight: "100vh", background: T.bg, color: T.text, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <button onClick={() => navigate(-1)} style={{ background: "none", border: "none", color: T.sub, cursor: "pointer", fontSize: 13, padding: 0, marginBottom: 28 }}>
          {isRTL ? "→" : "←"} {t("common.back").replace("← ", "").replace(" →", "")}
        </button>

        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontSize: 28, fontWeight: 800, margin: "0 0 6px", color: T.text }}>{t("common.proposals").replace("Proposals", "Contracts")}</h1>
          <p style={{ color: T.sub, margin: 0, fontSize: 14 }}>{counts.all} total · {counts.active} {t("contract.st.active")}</p>
        </div>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 24, flexWrap: "wrap" }}>
          {(["all", "active", "completed", "disputed"] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{
              ...
              {filterLabels[f] || f} ({counts[f]})
            </button>
          ))}
        </div>

        {loading ? (
          <div style={{ textAlign: "center", padding: 60, color: T.sub }}>{t("contract.loading")}</div>
        ) : filtered.length === 0 ? (
          <div style={{ textAlign: "center", padding: 60, background: T.surface, borderRadius: 16, border: `1px solid ${T.border}` }}>
            <div style={{ fontSize: 36, marginBottom: 12 }}>📄</div>
            <div style={{ fontSize: 15, fontWeight: 600, color: T.text, marginBottom: 6 }}>{t("prop.noProjects")}</div>
            <div style={{ fontSize: 13, color: T.sub }}>
              {filter === "all" ? t("contract.loading") : `${filterLabels[filter]}`}
            </div>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12, color: T.sub }}>Project #{c.project_id} · {t("contract.freelancer")} #{c.freelancer_id}</div>
                  ...
                  <span style={{ color: T.sub, fontSize: 16 }}>{isRTL ? "←" : "→"}</span>
                </div>
              );
            })}
          </div>
